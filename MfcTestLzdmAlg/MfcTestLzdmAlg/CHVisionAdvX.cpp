...
// === 保存 Double3DPointVec 到自定义二进制文件 ===
bool CHVisionAdvX::SaveDouble3DPointVecAsXYZMap(
    const std::string& filename,
    const Double3DPointVec& cloud,
    std::string& errorMsg)
{
    if (cloud.empty()) {
        errorMsg = "输入点云为空（HEIGHT=0）";
        return false;
    }

    uint32_t height = static_cast<uint32_t>(cloud.size());
    uint32_t width = static_cast<uint32_t>(cloud[0].size());

    // 校验：所有行宽度必须一致
    for (uint32_t y = 0; y < height; ++y) {
        if (cloud[y].size() != width) {
            errorMsg = "行 " + std::to_string(y) + " 的宽度(" + std::to_string(cloud[y].size()) +
                ") 与首行宽度(" + std::to_string(width) + ") 不一致";
            return false;
        }
    }
    ...
    uint64_t totalPoints = static_cast<uint64_t>(width) * height;

    std::ofstream file(filename, std::ios::binary);
    if (!file.is_open()) {
        errorMsg = "无法打开文件用于写入: " + filename;
        return false;
    }
    ...
    uint32_t pointCount = static_cast<uint32_t>(totalPoints);
    file.write(reinterpret_cast<const char*>(&pointCount), sizeof(pointCount));

    // 按行优先顺序写入 XYZ
    for (const auto& row : cloud) {
        for (const auto& pt : row) {
            file.write(reinterpret_cast<const char*>(&pt.X), sizeof(pt.X));
            file.write(reinterpret_cast<const char*>(&pt.Y), sizeof(pt.Y));
            file.write(reinterpret_cast<const char*>(&pt.Z), sizeof(pt.Z));
        }
    }

    if (file.fail()) {
        errorMsg = "写入文件时发生 I/O 错误";
        file.close();
        return false;
    }
...
// === 从文件加载为 Double3DPointVec ===
bool CHVisionAdvX::LoadDouble3DPointVecFromXYZMap(
    const std::string& filename,
    Double3DPointVec& cloud,
    std::string& errorMsg)
{
    std::ifstream file(filename, std::ios::binary);
    if (!file.is_open()) {
        errorMsg = "无法打开文件用于读取: " + filename;
        return false;
    }

    uint32_t width = 0, height = 0, pointCount = 0;
    file.read(reinterpret_cast<char*>(&width), sizeof(width));
    file.read(reinterpret_cast<char*>(&height), sizeof(height));
    file.read(reinterpret_cast<char*>(&pointCount), sizeof(pointCount));

    if (file.fail()) {
        errorMsg = "读取文件头部失败（可能文件损坏或非预期格式）";
        file.close();
        return false;
    }

    if (width == 0 || height == 0) {
        errorMsg = "文件头中 WIDTH 或 HEIGHT 为零";
        file.close();
        return false;
    }

    if (static_cast<uint64_t>(width) * height != pointCount) {
        errorMsg = "文件头中 WIDTH*HEIGHT 与 POINT_COUNT 不匹配";
        file.close();
        return false;
    }
    ...
    for (uint32_t y = 0; y < height; ++y) {
        for (uint32_t x = 0; x < width; ++x) {
            Point3D& pt = cloud[y][x];
            file.read(reinterpret_cast<char*>(&pt.X), sizeof(pt.X));
            file.read(reinterpret_cast<char*>(&pt.Y), sizeof(pt.Y));
            file.read(reinterpret_cast<char*>(&pt.Z), sizeof(pt.Z));

            if (file.fail()) {
                errorMsg = "读取点数据失败，位置 (x=" + std::to_string(x) +
                    ", y=" + std::to_string(y) + ")";
                cloud.clear();
                file.close();
                return false;
...
////== d3d转ply
//bool CHVisionAdvX::SaveDouble3DPointVecAsPlyWithHalcon(
//    const std::string& filename,
//    const Double3DPointVec& cloud,
//    std::string& errorMsg)
//{
...
//    {
//        if (cloud.empty()) {
//            errorMsg = "点云为空（HEIGHT=0）";
//            return false;
//        }
//
//        size_t height = cloud.size();
//        size_t width = cloud[0].size();
//
//        // 校验每行宽度一致
...
//
//        // 转为 Halcon HTuple
//        HTuple xTuple(xs.data(), xs.size());
//        HTuple yTuple(ys.data(), ys.size());
//        HTuple zTuple(zs.data(), zs.size());
//
//        // 创建 3D 对象模型（仅点云）
//        HTuple model3D;
//        GenObjectModel3dFromPoints(xTuple, yTuple, zTuple, &model3D);
//
//        // 保存为 PLY
//        // 注意：Halcon 默认导出 ASCII PLY；若需 binary，可指定参数（见下文）
//        WriteObjectModel3d(model3D, "ply", filename.c_str(), "","");
//
//        errorMsg.clear();
//        return true;
//
