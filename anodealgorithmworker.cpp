...
void AnodeAlgorithmWorker::processFinePositioningMemory(int blockIndex, const QList<s_Image3dS> &images)
{
    ...
    if (images.size() < 2) {
        emit finePositioningFailed(blockIndex, QStringLiteral("需要2个相机数据，当前只有1个"));
        return;
    }
    ...
    try {
        ...
        for (int iCam = 0; iCam < 2; iCam++) {
            ...
            if (rts.iCode != 0) {
                ...
                emit finePositioningFailed(blockIndex, QString("相机%1预处理失败: %2").arg(iCam + 1).arg(errorMsg));
                return;
            }
        }
        ...
        s_Rtnf accurateStatus = m_algorithm->OnAccurateAD(imgList, m_accuratePara, accurateResult);
        if (accurateStatus.iCode != 0) {
            QString msg = decodeAlgorithmMessage(accurateStatus.strInfo);
            if (msg.isEmpty()) {
        ...
        if (alignStatus.iCode != 0) {
            QString msg = decodeAlgorithmMessage(alignStatus.strInfo);
            if (msg.isEmpty()) {
        ...
        emit finePositioningFailed(blockIndex, QString("Halcon异常: %1").arg(ex.ErrorMessage().Text()));
    } catch (std::exception &ex) {
        emit finePositioningFailed(blockIndex, QString("异常: %1").arg(ex.what()));
    } catch (...) {
        emit finePositioningFailed(blockIndex, QStringLiteral("未知异常"));
    }
}
