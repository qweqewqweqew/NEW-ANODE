    ...
    QVBoxLayout *layoutReport = new QVBoxLayout(tabReport);
    {
        ...
        connect(sendBtn, &QPushButton::clicked, dlg, [=]() {
            ...
            for (const auto &v : arr) {
                ...
                if (!o.contains("x") || !o.contains("y") || !o.contains("z") || !o.contains("direction") || !o.contains("angle")) {
                    logMessage("输入无效：每个元素需含 x/y/z/direction/angle", "ERROR");
                    return;
                }
            }
...
void Widget::onBatchPositioningRequestCameraCapture(int blockIndex)
{
    ...
    // 使用相机1进行拍照（可以根据需要选择相机）
    if (m_camera1Controller && m_camera1Controller->isConnected()) {
        // 临时存储blockIndex，用于在imageReady回调中识别
        ...
        if (m_batchFinePositioningManager) {
            // 创建一个空的QImage，触发模拟逻辑（测试模式或非测试模式都会使用模拟结果）
            QImage emptyImage;
